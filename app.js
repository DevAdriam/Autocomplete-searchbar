...
let getinput = document.querySelector(".autocomplete");
let getitemscontainer = document.querySelector(".items-container");
...
getinput.addEventListener("keyup", (e) => {
  ...
  const hasproductstoshow = productfiltered.length > 0;

  getempty.style.display = "none";
  if (hasproductstoshow) {
    for (let i = 0; i < productfiltered.length; i++) {
      const productitemContainer = document.createElement("div");
      productitemContainer.id = productfiltered[i].id;
      productitemContainer.classList.add("productitemContainer");

      const productName = document.createElement("div");
      productName.classList.add("productName");
      productName.append(productfiltered[i].title);

      const productImage = document.createElement("img");
      productImage.classList.add("productImage");
      productImage.src = productfiltered[i].image;

      productitemContainer.append(productName, productImage);
      getitemscontainer.append(productitemContainer);
    }
  } else {
    getempty.style.display = "block";
